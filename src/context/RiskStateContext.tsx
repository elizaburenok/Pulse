...
export function RiskStateProvider({ children }: { children: ReactNode }) {
  ...
  // Переключение сценария — чистый лист: закрытые задачи сбрасываются.
  ...
  const value = useMemo<RiskStateValue>(() => {
    ...
    return { scenarioId, setScenarioId, scenario, levels, openTasks, closeTask, resetActions }
  }, [scenarioId, closedIds, setScenarioId, closeTask, resetActions])
