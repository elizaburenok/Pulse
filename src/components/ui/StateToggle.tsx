...
export default function StateToggle() {
  ...
  // страницу /accounting. «Серьёзно рискуете» (сценарий threat, Кейс 3) —
  // вход из интернет-банка, поэтому форсируем возврат на главную, чтобы не
  // унаследовать /accounting от предыдущего кейса. Для «Всё в порядке» точка
  // входа тоже интернет-банк, но навигацию не форсируем (остаёмся где были).
  function selectScenario(id: (typeof SCENARIO_ORDER)[number]) {
    setScenarioId(id)
    if (id === 'risk') navigate('/accounting')
    else if (id === 'threat') navigate('/')
  }
